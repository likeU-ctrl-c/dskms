...
function goods_thumb($goods = array(), $type = '') {
    ...
    if (in_array($upload_type['0'], array('alioss', 'cos'))) {
        ...
        $param=array();
        if($type){
            $param[]='resize,m_pad,w_'.$type.',h_'.intval(intval($type)*125/225);
        }
        //是否有水印
...
function goods_bgthumb($file, $type = '', $store_id = false)
{
    ...
    if (in_array($upload_type['0'], array('alioss', 'cos'))) {
        ...
        $aliendpoint_type = config('ds_config.aliendpoint_type');
        if($aliendpoint_type) {
            return HTTP_TYPE.config('ds_config.alioss_endpoint') . '/' . ATTACH_GOODS . '/' . $store_id . '/' . $date . '/' . $file;
        }else{
            return 'https://'.config('ds_config.alioss_bucket').'.'.config('ds_config.alioss_endpoint') . '/' . ATTACH_GOODS . '/' . $store_id . '/' . $date . '/' . $file;
        }
    }
