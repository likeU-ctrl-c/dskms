...
class Pxianshi extends BaseModel {
    ...
    public function editExpireXianshi($condition) {
        ...
        if ($result) {
            foreach ($xianshigoods_list as $value) {
                $this->_unlockGoods($value['goods_id']);
            }
        }
        return true;
    }
    ...
    private function _unlockGoods($goods_id)
    {
        ...
        // 添加对列 更新商品促销价格
        model('cron')->addCron(array('cron_exetime'=>TIMESTAMP,'cron_type'=>'updateGoodsPromotionPriceByGoodsId','cron_value'=>serialize($goods_id)));
    }
