...
class  Voucher extends BaseModel
{
    ...
    public function getVoucherquotaCurrent($store_id)
    {
        ...
        $where = array();
        $where[]=array('voucherquota_storeid','=',$store_id);
        $where[]=array('voucherquota_endtime','>', TIMESTAMP);
        $info = Db::name('voucherquota')->where($where)->find();
        return $info;
    }
    ...
    public function getCanChangeTemplateInfo($vid, $member_id, $store_id = 0,$no_point=false)
    {
        ...
        $where[] = array('vouchertemplate_enddate','>', time());
        ...
        $condition = array();
        $condition[] = array('voucher_owner_id','=',$member_id);
        $condition[] = array('voucher_store_id','=',$template_info['vouchertemplate_store_id']);

        $voucher_list = $this->getVoucherList($condition);
        //halt($voucher_list);
        if (!empty($voucher_list)) {
            if(!$no_point){
