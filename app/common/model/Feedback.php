...
public $page_info;
    ...
    public function getFeedbackList($condition, $pagesize = null, $order = 'fb_id desc') {
        if($pagesize){
        $result = Db::name('feedback')->where($condition)->order($order)->paginate(['list_rows'=>$pagesize,'query' => request()->param()],false);
        $this->page_info=$result;
        $list = $result->items();
        }else{
            $list = Db::name('feedback')->where($condition)->order($order)->select()->toArray();
        }
        return $list;
    }
