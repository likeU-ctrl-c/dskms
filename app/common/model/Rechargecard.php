...
class  Rechargecard extends BaseModel
{
    ...
    {
        return Db::name('rechargecard')->where(array('rc_id' => (string) $id,))->update(array('rc_tsused' => time(), 'rc_state' => 1, 'member_id' => $memberId, 'member_name' => $memberName,));
    }
    ...
    {
        $array = Db::name('rechargecard')->field('rc_sn')->where(array(
                                               array('rc_sn','in', $sns),
                                           ))->select()->toArray();
