...
//        $add_time_from = strtotime(date("Y-m-d", (strtotime(date("Y-m-d")) - 60 * 60 * 24 * 30)));   //30天前
        ...
        // 出售中的商品
        $goods_online = $goods_model->getGoodsOnlineCount(array(array('store_id','=',session('store_id'))));
        if (config('ds_config.goods_verify')) {
            // 等待审核的商品
            $goods_waitverify = $goods_model->getGoodsWaitVerifyCount(array(array('store_id','=',session('store_id'))));
            // 审核失败的商品
            $goods_verifyfail = $goods_model->getGoodsVerifyFailCount(array(array('store_id','=',session('store_id'))));
        }
        // 仓库待上架的商品
        $goods_offline = $goods_model->getGoodsOfflineCount(array(array('store_id','=',session('store_id'))));
        // 违规下架的商品
        $goods_lockup = $goods_model->getGoodsLockUpCount(array(array('store_id','=',session('store_id'))));
        // 等待回复商品咨询
