...
class  Memberinviter extends BaseMember {
    ...
    public function add(){
        ...
        if(config('ds_config.inviter_condition')){
            //检查消费金额
            $temp=Db::name('vrorder')->where('buyer_id='.session('member_id').' AND order_state='.ORDER_STATE_PAY)->field('SUM(order_amount) AS order_amount,SUM(refund_amount) AS refund_amount')->find();
