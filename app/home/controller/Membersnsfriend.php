...
class  Membersnsfriend extends BaseMember {
    ...
    public function findlist() {
        ...
        $condition[] = array('member_name','like','%' . $searchname . '%');
        // 省份
        $member_provinceid = intval(input('post.provinceid'));
        if ($member_provinceid > 0) {
            $condition[] = array('member_provinceid','=',$member_provinceid);
        }
        // 城市
        $member_cityid = intval(input('post.cityid'));
        if ($member_cityid > 0) {
            $condition[] = array('member_cityid','=',$member_cityid);
        }
        // 地区
        $member_areaid = intval(input('post.areaid'));
        if ($member_areaid > 0) {
            $condition[] = array('member_areaid','=',$member_areaid);
        }
        // 性别
        $member_sex = intval(input('post.sex'));
        if ($member_sex > 0) {
            $condition[] = array('member_sex','=',$member_sex);
        }
        // 年龄
        ...
        if ($member_birthday > 0) {
            switch ($member_birthday) {
                case 1:
                    ...
                    $condition[] = array('member_birthday','BETWEEN', $s_time . ',' . $e_time);
                    break;
                case 2:
                ...
                case 5:
                    ...
                    $condition[]=array('member_birthday','<=', $e_time);
                    break;
            }
        }

        $memberlist = Db::name('member')->where($condition)->limit(50)->select()->toArray();
        if (!empty($memberlist)) {
    ...
    public function batch_addfollow() {
        ...
        // 将被关注会员列表
        $pm_array = Db::name('member')->where('member_id','in',$ids)->select()->toArray();

        // 查询是否已经关注
        $condition = array();
        $condition[] = array('friend_frommid','=',session('member_id'));
        $condition[] = array('friend_tomid','in',$ids);
        $gz_array = Db::name('snsfriend')->where($condition)->select()->toArray();
        $gz_array = array_under_reset($gz_array, 'friend_tomid', 1);
        // 查询对方是否关注我
        $condition = array();
        $condition[] = array('friend_tomid','=',session('member_id'));
        $condition[] = array('friend_frommid','in',$ids);
        $bgz_array = Db::name('snsfriend')->where($condition)->select()->toArray();
