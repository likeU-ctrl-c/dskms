...
class Exampaper extends AdminControl {
    ...
    public function initialize() {
        parent::initialize();
        Lang::load(base_path() . 'admin/lang/' . config('lang.default_lang') . '/exampaper.lang.php');
    }
    ...
    public function index() {
        ...
        if (!empty($exampaper_name)) {
            $condition[] = array('exampaper_name','like', "%" . $exampaper_name . "%");
        }
        $exampaper_type = input('param.exampaper_type');
        if ($exampaper_type != '') {
            $condition[] = array('exampaper_type','=',$exampaper_type);
        }
    ...
    public function config() {
        ...
        if (!request()->isPost()) {
            ...
            View::assign('exambank_level_list', $exambank_model->getLevelList()); #难度等级
            View::assign('examtype_list', $exambank_model->getExamtypeList()); #题目类型
            ...
            if ($exampaper['exampaper_type'] == 0) {
                //普通试卷
                return View::fetch('config_0');
            } else {
                //随机试卷
                return View::fetch('config_1');
            }
        } else {
            $params = input('param.');
                ...
                foreach ($params['args'] as $key => $value) {
                    ...
                    if (isset($value['exambank_score'])) {
                        foreach ($value['exambank_score'] as $exambank_score_key => $exambank_score_value) {
                            ...
                            $exambank = Db::name('exambank')->where('exambank_id', $exambank_id)->find();
    ...
    public function exampaperlog() {
        ...
        View::assign('show_page', $exampaperlog_model->page_info->render());
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            ...
            array(
                ...
                'url' => "javascript:dsLayerOpen('" . url('exampaper/add') . "','添加试卷')"
            ),
        );
