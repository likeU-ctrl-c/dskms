...
class Examclass extends AdminControl {
    ...
    public function initialize() {
        parent::initialize();
        Lang::load(base_path() . 'admin/lang/' . config('lang.default_lang') . '/examclass.lang.php');
    }
    ...
    public function add() {
        ...
        if (request()->isPost()) {
            ...
            $examclass_validate = ds_validate('examclass');
            ...
            View::assign('examclass_parent_id', intval(input('param.examclass_parent_id')));
    ...
    public function ajax() {
        ...
        switch (input('param.branch')) {
            ...
            case 'examclass_name':
                ...
                $condition[]=array('examclass_name','=',trim(input('param.value')));
                $condition[]=array('examclass_parent_id','=',$class_array['examclass_parent_id']);
                $condition[]=array('exambanksuggest_id','<>',intval(input('param.id')));
                $class_list = $examclass_model->getExamclassList($condition);
                if (empty($class_list)) {
                    $condition = array();
                    $condition[]=array('examclass_id','=',intval(input('param.id')));
                    $update_array = array();
                    $update_array['examclass_name'] = trim(input('param.value'));
                    $examclass_model->editExamclass($condition,$update_array);
                    echo 'true';
                    exit;
                } else {
                    echo 'false';
            ...
            case 'check_class_name':
                $examclass_model = model('examclass');
                $condition[]=array('examclass_name','=',trim(input('param.examclass_name')));
//                $condition[]=array('examclass_parent_id','=',intval(input('param.examclass_parent_id')));
                $condition[]=array('examclass_id','<>',intval(input('param.examclass_id')));
